...
async function loadStoreData() {
  ...
  try {
    const [coins, ranks] = await Promise.all([
      fetch(`${API_URL}/shop/coins`).then(r => r.json()).catch(() => []),
      fetch(`${API_URL}/shop/ranks`).then(r => r.json()).catch(() => [])
    ]);
    
    storeData = { coins, ranks };
    renderStoreProducts();
  } catch (err) {
    grid.innerHTML = '<p style="text-align:center;color:var(--text-3);">Unable to load products. Please try again later.</p>';
  }
}
...
function renderStoreProducts() {
  ...
  storeData.ranks.forEach(rank => {
    allProducts.push({
      id: rank._id,
      type: 'ranks',
      title: rank.rank_name,
      price: rank.price,
      image: rank.image_url || 'assets/images/logo.png',
      description: rank.perk_description,
      badge: rank.badge_color,
      duration: rank.duration_days
    });
  });
  
  storeData.coins.forEach(coin => {
    allProducts.push({
      id: coin._id,
      type: 'coins',
      title: coin.package_name,
      price: coin.price,
      image: coin.image_url || 'assets/images/logo.png',
      description: coin.description,
      coins: coin.coin_amount,
      gamemode: coin.gamemode
    });
  });
  
  if (allProducts.length === 0) {
    grid.innerHTML = '<p style="text-align:center;color:var(--text-3);">No products available at the moment.</p>';
    return;
  }
  
  grid.innerHTML = allProducts.map(product => {
    const isCoins = product.type === 'coins';
    const priceDisplay = `$${product.price.toFixed(2)} USD`;
    const badgeStyle = product.badge ? `border-left: 3px solid ${product.badge};` : '';
    
    return `
      <div class="card package-card reveal" data-category="${product.type}" data-product-id="${product.id}" data-product-title="${product.title}" data-product-price="${product.price}" data-product-image="${product.image}" data-product-description="${product.description || ''}" style="${badgeStyle}">
        <img src="${product.image}" alt="${product.title}" class="package-img">
        <div class="package-title">${product.title}</div>
        <div class="package-price">${priceDisplay}</div>
        <div class="package-actions">
          <a href="#" class="btn-icon" title="View details">
            <i class="ti ti-info-circle"></i>
          </a>
          <a href="./cart.html" class="btn-primary add-to-cart"><i class="ti ti-plus"></i> Add to cart</a>
        </div>
      </div>
    `;
  }).join('');
...
function initProductCards() {
  ...
  document.querySelectorAll('.package-card .btn-icon').forEach(btn => {
    btn.addEventListener('click', (e) => {
      e.preventDefault();
      const card = btn.closest('.package-card');
...
function showProductModal(card) {
  ...
  openProductModal({ id, title, price, image, description });
}
